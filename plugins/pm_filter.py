...
async def language_check(bot, query):
    ...
    if files:
        ...
        return await query.answer(f"Sorry, No Files Found For Query {movie}.", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Globle Filter', callback_data='global_filters')
        ]]
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Contact", url="t.me/skcontactbot")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            files, offset, total_results = await get_search_results(message.message.chat.id ,search.lower(), offset=0, filter=True)
            if not files:
...
async def advantage_spell_chok(client, msg):
    ...
    btn.append([InlineKeyboardButton(text="Close", callback_data=f'spol#{reqstr1}#7#close_spellcheck')])
